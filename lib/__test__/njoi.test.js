...
describe("njoi", () => {
  ...
  xit("should a sensible description", () => {
  ...
  xit("should treat strings with defaults as an other type", () => {
    const schema = Joi.object({ foo: Joi.string().default("bar") });
    expect(njoi.jsonish()(schema)).toMatchSnapshot();
  });

  xit("should treat uris correctly", () => {
    const schema = Joi.object({ foo: Joi.string().uri().required() });
    expect(njoi.jsonish()(schema)).toMatchSnapshot();
  });
});
